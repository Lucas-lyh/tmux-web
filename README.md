...
一个浏览器，串起 tmux、远程节点和 AI Agent 的工作流。
...
| 📦 文件直达 | 拖拽上传、路径下载；CLI 同样支持本机与节点传文件 |
| 📊 状态速览 | CPU、内存、GPU、网络、磁盘，以及本机 Codex / Kimi Token 统计 |
| 🪄 Agent 搭档 | 自带 CLI、HTTP / WebSocket API 和两个可复用 skills |
...
`run` 通过完成标记等待命令结果，最多读取最近 300 行；全屏程序请使用网页终端。完整命令见 `.venv/bin/python client.py --help`。

仓库自带 [tmux-web-api](skills/tmux-web-api/SKILL.md)（操作会话 / 传文件）和 [tmux-web-page](skills/tmux-web-page/SKILL.md)（发布临时报告）。安装到 Codex：

```bash
mkdir -p "${CODEX_HOME:-$HOME/.codex}/skills"
cp -R skills/tmux-web-api skills/tmux-web-page "${CODEX_HOME:-$HOME/.codex}/skills/"
```

在新会话中使用，并告诉 Agent 项目的实际路径。其他支持 `SKILL.md` 的工具也可按各自方式加载。

### 接入远程节点

远程机器仅需 **Linux / Python 3.10+**，无需 tmux 或第三方 Python 包。将 `node.py` 和主机启动时生成的 `.node-secret` 通过 SSH 等可信通道复制到节点，把密钥存为权限 `600` 的 `~/.tmux-web-node-secret`，然后运行：
...
连接后即可在网页或 CLI 中使用 `gpu1:train`。节点密钥拥有完整管理权限，请保存在版本控制之外；节点目前仅支持 `ws://`，跨公网接入请使用 VPN 或 SSH 隧道。主机重启后节点会自动重连；**节点进程退出或节点机器重启会丢失其会话**。
