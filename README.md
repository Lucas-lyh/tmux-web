...
节点自动读取 `http_proxy` / `HTTP_PROXY`，未设置时使用 `all_proxy` / `ALL_PROXY`；同名变量以小写优先。`no_proxy` / `NO_PROXY` 支持主机、域名后缀、可选端口和 `*`，匹配时直接连接。由于节点地址是 `ws://`，不使用 `https_proxy`。例如先执行以下命令，再运行节点连接命令：

```bash
export http_proxy=http://10.20.9.2:3128
export no_proxy=localhost,127.0.0.1,::1
```

支持 `http://` 代理及 HTTP CONNECT 隧道，不支持 HTTPS/SOCKS 代理。需要认证时使用 `http://用户名:密码@代理地址:端口`（特殊字符需 URL 编码），通过 Basic 代理认证；代理凭据仅发送给代理，不转发给主服务。代理必须允许 CONNECT 到主服务端口，失败时日志会显示状态码（例如 403、407），不会输出凭据或代理响应正文。配置代理后连接失败不会回退直连。
