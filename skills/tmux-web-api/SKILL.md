...
只有用户任务需要时才部署新节点。节点只需要原有的 Linux / Python 3.10+，无需 tmux、pip、加密库或其他工具。加密实现内置在单个 `node.py` 中，不会自动下载或安装依赖。
...
先更新主服务，再更新节点；旧节点兼容通道不会自动获得加密。节点连接固定用 `ws://`，不使用 TLS；不要增加证书或 CA 配置。`TMUX_WEB_NODE_TOKEN` 和 `--token` 仍受支持，密钥文件方式能避免在命令参数里存放凭据。
